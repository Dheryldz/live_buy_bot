...
// Route-specific raw parser to preserve exact body for HMAC check
app.post('/webhook/public-sale', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    ...
    // 3) Only process purchase.confirmed
    if (data.event !== 'purchase.confirmed') {
...
app.listen(PORT, () => {
  console.log(`Live Buy Bot listening on http://localhost:${PORT}`);
});
